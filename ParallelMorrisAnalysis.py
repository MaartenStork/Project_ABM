...
import ParallelDynamicCoop as dc
import parameters
...
def process_sample(sample_data):
    """
    Helper function to process a single Morris or Sobol sample
    for parallel execution.
    
    Args:
        sample_data: Tuple containing (index, parameter values, parameter names)
        
    Returns:
        Tuple: (index, model output)
    """
    # Unpack the data
    i, xi, param_names = sample_data
...
def morris_analysis(param_names, bounds, original_values, num_trajectories=200, grid_levels=10, parallel=True):
    """Morris screening with statistically significant sample size"""
    # Configure problem and generate samples
    problem = {'num_vars': len(param_names), 'names': param_names, 'bounds': bounds}
    ...
    use_parallel = parallel and len(X) >= num_processors * 5  # Only use parallel if enough samples
    
    if use_parallel:
        # Prepare data for parallel processing
        sample_data = [(i, xi, param_names) for i, xi in enumerate(X)]
        
        print(f"Using parallel processing with {num_processors} workers for {len(X)} Morris samples")
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_processors) as executor:
            # Submit tasks
            futures = [executor.submit(process_sample, data) for data in sample_data]
            
            # Process results as they complete
            for _ in tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc="Morris runs (parallel)"):
                pass
        ...
        print("Using sequential processing for Morris runs")
        for i, xi in enumerate(tqdm(X, desc='Morris runs (sequential)')):
            for name, val in zip(param_names, xi): 
                setattr(parameters, name, val)
            Y[i] = run_model()
    
    end_time = time.time()
    print(f"Morris analysis completed in {end_time - start_time:.2f} seconds using {'parallel' if use_parallel else 'sequential'} processing")
