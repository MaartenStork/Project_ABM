...
def process_fish(fish_ag, all_agents, fish_list_len, new_fish_count, reproduction_check=True):
    """
    Process a single fish agent - handles movement and potentially reproduction
    Returns a tuple (new_fish_agent_if_reproduced_or_None, species_subtype)
    """
    species_subtype = fish_ag.subtype
    
    # Calculate zones
    repulsion, alignment, attraction = init_boids_zones(fish_ag, all_agents, rad_repulsion_sqr, rad_orientation_sqr,
                                                     rad_attraction_sqr)

    # Handle movement based on zones
    if len(repulsion) > 0:
    ...
    elif all([len(repulsion) == 0, len(alignment) == 0, len(attraction) > 0]):
    ...
    # Check reproduction
    new_fish_agent = None
    if reproduction_check and fish_list_len + new_fish_count < total_fish_count[0]:
        reproduction_chance = fish_ag.reproduction_rate * (1-sum([1 for j in all_agents if j.type == 'fish'])/float(K))
        if rd.random() < reproduction_chance:
            # Create a new fish agent similar to reproduction function but without modifying global state
            new_fish_agent = agent()
            new_fish_agent.type = "fish"
            new_fish_agent.subtype = fish_ag.subtype
            new_fish_agent.x = fish_ag.x
            new_fish_agent.y = fish_ag.y
            new_fish_agent.speed = fish_ag.speed
            new_fish_agent.dx = fish_ag.dx
            new_fish_agent.dy = fish_ag.dy
            new_fish_agent.reproduction_rate = fish_ag.reproduction_rate
    
    return (new_fish_agent, species_subtype)
...
def update_fish():
    ...
    # Create a copy of agents to avoid concurrent modification issues
    agents_snapshot = list(agents)
    new_fish = 0
    new_species_count = {}
    new_fish_agents = []
    
    # Determine the optimal chunk size (smaller chunks for more fish)
    num_fish = len(fish_list)
    num_processors = min(multiprocessing.cpu_count(), 8)  # Limit to 8 processors max
    
    # Initialize species count for existing species
    for fish_ag in fish_list:
        if fish_ag.subtype not in new_species_count:
            new_species_count[fish_ag.subtype] = 0
        new_species_count[fish_ag.subtype] += 1
    
    if num_fish >= num_processors * 10:  # Only parallelize if we have enough fish
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_processors) as executor:
            # Map the process_fish function to each fish in the list
            futures = []
            for fish_ag in fish_list:
                # Submit each fish for processing
                future = executor.submit(process_fish, fish_ag, agents_snapshot, len(fish_list), new_fish, True)
                futures.append(future)
                
            # Collect results as they complete
            for future in concurrent.futures.as_completed(futures):
                new_fish_agent, species_subtype = future.result()
                if new_fish_agent:
                    new_fish_agents.append(new_fish_agent)
                    new_fish += 1
                    if species_subtype in new_species_count:
                        new_species_count[species_subtype] += 1
                    else:
                        new_species_count[species_subtype] = 1
    else:
        # For smaller numbers of fish, use the sequential approach
        for fish_ag in fish_list:
            new_fish_agent, _ = process_fish(fish_ag, agents_snapshot, len(fish_list), new_fish, True)
            if new_fish_agent:
                new_fish_agents.append(new_fish_agent)
                new_fish += 1
    ...
    # Add any new fish to the main agents list
    for new_fish_agent in new_fish_agents:
        agents.append(new_fish_agent)
    ...
    end_time = time.time()
    print(f"Fish update completed in {end_time - start_time:.3f} seconds with {'parallel' if len(fish_list) >= 10 * multiprocessing.cpu_count() else 'sequential'} processing")
...
def update_one_unit_time():
    global time1, agents, fish, total_fish_count
    ...
    # Process fishers in parallel
    fisher_list = [j for j in agents if j.type == 'fishers']
    ...
    if num_fishers >= num_processors * 5:  # Only parallelize if we have enough fishers
        # Use ProcessPoolExecutor for parallelization
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_processors) as executor:
            # Map the process_fisher function to each fisher
            futures = []
            for fisher in fisher_list:
                future = executor.submit(process_fisher, fisher, agents_snapshot)
                futures.append(future)
                
            # Process results as they complete
            for i, future in enumerate(concurrent.futures.as_completed(futures)):
                fisher, caught_fish, did_catch = future.result()
                
                # Update fisher state
                fisher_list[i].harvest = fisher.harvest
                
                # Update data structures
                total_hav_data[fisher_list[i].num].append(fisher_list[i].harvest)
                current_hav_data[fisher_list[i].num].append(did_catch)
                
                # Record caught fish for later removal
                if caught_fish:
                    caught_fish_list.append(caught_fish)
    else:
        # For smaller numbers of fishers, use the sequential approach
        for fisher in fisher_list:
            # Calculate catch
    ...
    # Remove caught fish from the agents list
    for fish in caught_fish_list:
        if fish in agents:  # Check if fish is still in agents list
            agents.remove(fish)
            
    # Timing measurement - end
    fisher_end_time = time.time()
    print(f"Fisher update completed in {fisher_end_time - fisher_start_time:.3f} seconds with {'parallel' if len(fisher_list) >= 5 * multiprocessing.cpu_count() else 'sequential'} processing")
