...
K = 50 # carrying capacity of fishing ground
n = 9 # number of simulation time steps
...
def delete_prev_sim():
    ...
    pattern = os.path.join(script_dir, "year_*.png")
...
def initialize():
    ...
    total_fish_count = [K]  # list containing number of fishes
...
def init_fish_agents():
    fish_params = init_fish(carrying_capacity=K)
...
def observe():
    ...
    if len(fish) > 0:
        ...
        for i, subtype in enumerate(subtypes):
            ...
            plt.plot(X, Y, '^', color=colors[i], markersize=3, label=f'fish {subtype}')
...
def plot_summary():
    ...
    for i in range(num_seasons):
        ...
        plt.text((season_start + season_end) / 2, max(total_fish_count) * 0.9, season_name, horizontalalignment='center', verticalalignment='center')
    plt.xlabel('Time')
